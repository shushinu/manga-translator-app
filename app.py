...
# ======================================================
    ...
    if "corrected_text" not in st.session_state:
        ...
        def _create_log_only_here(sb_client, combined_text: str):
            """
            只給『💾 儲存並產生提示內容』按鈕使用：
            - 僅在目前沒有 log_id 且 combined_text 有內容時，才 insert 建新列
            - 成功後把 id 存到 st.session_state['log_id']
            """
            ...
            res = (
                ...
                .insert({
                    "user_id": "guest",           # TODO: 導入登入後改為真實 user_id
        ...
        def _update_prompt_if_possible(sb_client):
            """已有 log_id 且有內容時，更新同筆 combined_prompt；不會新建"""
        ...
        def _update_output_if_possible(sb_client):
            """已有 log_id 且有譯文時，更新同筆 output_text；不會新建"""
            log_id = st.session_state.get("log_id")
...
【翻譯方針】\n{st.session_state['translation_policy']}\n\n"""
        ...
        if st.button("💾 儲存提示內容"):
            ...
            # ❌ 不建新列；僅在已有 log_id 時更新同一筆 combined_prompt
            try:
        ...
        if st.button("執行翻譯"):
            ...
            if not prompt_for_translation:
                ...
                # ❌ 不建新列；僅在已有 log_id 時回填同一筆 output_text
                try:
                    if st.session_state.get("log_id"):
